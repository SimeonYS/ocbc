...
class OocbcPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ocbc.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ocbc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ocbc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ocbc (date, title, link, content)"
